...
import { demanderReinitialisationMotDePasse, type EtatFormulaireAuth } from "@/lib/auth/actions";

const ETAT_INITIAL: EtatFormulaireAuth = {};

export function ResetPasswordForm() {
  const [etat, action, enCours] = useActionState(demanderReinitialisationMotDePasse, ETAT_INITIAL);
  ...
  return (
    <form action={action} className="flex w-full max-w-sm flex-col gap-4">
      ...
      {etat?.message && <p className="text-sm text-zinc-700 dark:text-zinc-300">{etat.message}</p>}
